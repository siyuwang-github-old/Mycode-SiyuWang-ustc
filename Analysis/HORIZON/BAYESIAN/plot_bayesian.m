classdef plot_bayesian < plot_horizonn
    ...
    methods
        ...
        function loadbayes(obj, filename, issample)
            disp('loading stats');
            files = dir(fullfile(obj.siyupathresultbayes,[filename '*_bayesresult.mat']));
            for fi = 1:length(files)
                stats = importdata(fullfile(files(fi).folder, files(fi).name));
                obj.stats.(stats.modelname) = stats.stats;
                obj.time_bayesianfit = stats.tictoc;
            end
            if exist('issample') && issample == 1
                disp('loading samples');
                obj.samples = importdata(fullfile(obj.siyupathresultbayes, [filename, '_bayessamples.mat']));
            end
            disp('loaded');
        end
