classdef cardstopping < SiyuStatistics & SiyuPlots
    ...
    methods
        function obj = cardstopping()
            ...
            obj.colorh = {obj.colors.AZred,obj.colors.AZsand,obj.colors.AZcactus,...
                obj.colors.AZriver,obj.colors.AZblue, obj.colors.AZblue*1.5, ...
        ...
        function loadall(obj, excludecriteria)
            ...
            obj.data{3} = obj.load(filename, excludecriteria);
            obj.isxfit = false;
            disp('loaded');
        end
        function dataraw = load(obj, filename, excludecriteria)
            %             obj.data = obj.celltoarray(data);
            data = importdata(filename);
            data = obj.completedatasets(data);
            dataraw = obj.exclude_performance(data, excludecriteria, length(obj.data) + 1);
        end
        function dataraw = exclude_performance(obj, data, thres, suffixidx)
            if nargin < 3
                option = '';
            end
            ...
            %              if strcmp(option, 'EXP1')
            %                  percent_raw{12} = percent_raw{12}(end-1:end)
            %              end
            ...
            ylims = ylim;
            %             thresy = 80;
            %             line([thresy, thresy], [ylims(1) ylims(2)], 'Color', 'k','LineWidth', obj.linewidth,'LineStyle','--');
            ...
            %              end
            %             idx_reward = (reward_mean >= thres)' & (percent >= 80);%| isnan(percent)
            %             data = data(idx_reward);
            %             ages = arrayfun(@(x)obj.getnum(x.demo.age), data);
            
            %             dataraw = data(ages >= 18);
            dataraw = data;
        ...
        function computeOptimalThreshold(obj)
         ...
         function default_analysis_new(obj)
        ...
        function plot_choicecurve_full(obj)
            for expi = 1:length(obj.data)
                horizons = obj.horizons{expi};
                gp = obj.gpfull{expi};
                avs = obj.avsfull{expi};
                obj.figure;
                hh = 1:length(horizons);
                thorizons = horizons(hh);
                obj.now_color = obj.colorh(thorizons);
                legs = arrayfun(@(x)['Horizon = ',num2str(x)],thorizons,'UniformOutput',false);
                av = avs.av.choicecurvefull;
                err = avs.err.choicecurvefull;
                obj.new;
                obj.leglist = [];
                obj.lineplot(av,err,mean(obj.avs{expi}{max(horizons)}.xbins_reward',1));
                ylim([0 1]);
                obj.temp_legloc = 'Northwest';
                obj.legend(legs);
                xlabel('reward');
                ...
                set(gca, 'FontSize',obj.fontsize_face);
                set(gca, 'XTick', [0:20:100]);
                obj.save(['Choicecurve_full_', obj.savesuffixs{expi}]);
            end
        end
