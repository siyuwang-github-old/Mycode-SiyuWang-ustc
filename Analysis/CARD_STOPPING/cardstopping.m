classdef cardstopping < SiyuStatistics & SiyuPlots
    ...
    methods
        ...
        function dataraw = exclude_performance(obj, data, thres, suffixidx)
            ...
            reward_time = arrayfun(@(x)obj.getcolumn(x.task.reward_s,200)', data, 'UniformOutput', false)';
            ...
            %              end
            idx_reward = (reward_mean >= thres) | (percent >= 80)';
        ...
        function data = completedatasets(obj, data)
            idx = arrayfun(@(x)length(x.task.nstop_s) == length(x.task.horizon), data);
            data = data(idx);
        end
        ...
        function default_analysis(obj, issimu)
            if nargin < 2
                issimu = 0;
            end
            ...
            for di = 1:length(obj.data)
                data = obj.data{di};
                %             obj.main_simulation;
                horizons = arrayfun(@(x)x.task.horizon,data,'UniformOutput',false);
                horizons = unique(horzcat(horizons{:}));
